...
Use the current stable Flutter SDK (Dart 3.10 or newer):
...
The Android bootstrap downloads the pinned Gradle 8.14 distribution on its first build and verifies it against Gradle's published SHA-256 checksum. Local release APKs use the Android debug signature so they remain installable for the hackathon demo. For a distributable release, set all four signing variables before building:
...
Never commit the keystore or passwords. These signing values are not service API keys.
...
- `POST /api/v1/risk/score` sends the backend `PaymentDetails` shape (`vpa`, `payee_name`, `amount`, `transaction_note`, `currency`, and `transaction_reference`) with an anonymous `device_id` and optional structured context. It retains the returned `assessment_id` for response preparation.
- Each signal has `code`, `label`, integer `weight`, and `evidence`.
- `POST /api/v1/context/analyze` accepts selected text and an optional size/type-checked PNG, JPEG or WebP screenshot. `consent_to_external_ai` is false unless the user explicitly enables Gemini. Local text rules and deterministic payment scoring remain available when Gemini is disabled or over quota.
