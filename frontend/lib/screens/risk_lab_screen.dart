...
class _RiskLabScreenState extends State<RiskLabScreen> {
  DemoScenario _selected = DemoRepository.bundled.first;
        ...
        children: <Widget>[
          ...
          LayoutBuilder(
            builder: (BuildContext context, BoxConstraints constraints) {
              final Widget rail = _ScenarioRail(
                selectedId: _selected.id,
                onSelected: _select,
              );
              final Widget inspector = AnimatedSwitcher(
              ...
              return Column(
                ...
                children: <Widget>[rail, const SizedBox(height: 22), inspector],
              );
            },
          ),
        ],
  ...
  void _select(DemoScenario scenario) {
    if (_selected.id == scenario.id) return;
...
class _ScenarioRail extends StatelessWidget {
  const _ScenarioRail({required this.selectedId, required this.onSelected});
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: <Widget>[
      Text('Demo outcomes', style: Theme.of(context).textTheme.titleLarge),
          ...
          children: DemoRepository.bundled.indexed
              .expand(
                ((int, DemoScenario) item) => <Widget>[
                  ...
                  _ScenarioSelector(
                    key: Key('risk_lab_case_${item.$2.id}'),
                    scenario: item.$2,
                    selected: item.$2.id == selectedId,
...
class _ScenarioSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    ...
    selected: selected,
    label:
        '${scenario.title}, ${scenario.assessment.level.label}, score ${scenario.assessment.score} of 100',
    child: Material(
